    ...
    try {
        ...
        if (session.role === "ADMIN") {
            const [totalEmployee, todayAttendance,pendingLeaves] = await Promise.all([
                employModel.countDocuments({isDeleted:{$nor:true}}),
            ...
            return res.json({role: 'ADMIN',totalEmployee,totalDepartments:DEPARTMENTS.length ,todayAttendance,pendingLeaves});
        }else{
            ...
            const [currentMonthAttendance,pendingLeaves,latestPayslips] = await Promise.all([
                attendanceModel.countDocuments({employeeId: employee._id,
        ...
        payslipModel.findOne({employeeId: employee._id}).sort({createdAt:-1}).lean()
            ...
            return res.json({role:'EMPLOYEE' , employee: {...employee,id:employee._id.toString()},currentMonthAttendance,pendingLeaves,latestPayslips: latestPayslips? {...latestPayslips,id:latestPayslips._id.toString() }: null})
